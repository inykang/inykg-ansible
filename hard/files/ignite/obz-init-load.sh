...
if [ -z "$JVM_OPTS" ] ; then
    if [[ `"$JAVA" -version 2>&1 | egrep "1\.[7]\."` ]]; then
        JVM_OPTS="-Xms1g -Xmx6g -server -XX:+AggressiveOpts -XX:MaxPermSize=256m"
    else
        JVM_OPTS="-Xms1g -Xmx6g -server -XX:+AggressiveOpts -XX:MaxMetaspaceSize=256m"
    fi
fi
...
#    MAIN_CLASS=org.apache.ignite.startup.cmdline.CommandLineStartup
    MAIN_CLASS=com.obzen.ignite.loader.ObzInitialLoader
fi
