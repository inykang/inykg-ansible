...
CP="${IGNITE_LIBS}"
CP="${CP}:${IGNITE_HOME_TMP}/caches/:${IGNITE_HOME_TMP}/config"
#CP="${CP}:${IGNITE_HOME_TMP}/libs/obzen-ignite-statistics-0.9.9.jar"
...
if [ -z "$JVM_OPTS" ] ; then
    if [[ `"$JAVA" -version 2>&1 | egrep "1\.[7]\."` ]]; then
        JVM_OPTS="-Xms1g -Xmx5g -server -XX:+AggressiveOpts -XX:MaxPermSize=256m"
    else
        JVM_OPTS="-Xms1g -Xmx5g -server -XX:+AggressiveOpts -XX:MaxMetaspaceSize=512m"
    fi
fi
...
#    MAIN_CLASS=org.apache.ignite.startup.cmdline.CommandLineStartup
    MAIN_CLASS=com.obzen.ignite.adminrest.IgniteRest
fi
...
do
    if [ "${INTERACTIVE}" == "1" ] ; then
        case $osname in
            ...
            ;;
            *)
                nohup "$JAVA" ${JVM_OPTS} ${QUIET} "${RESTART_SUCCESS_OPT}" ${JMX_MON} \
                 -DIGNITE_HOME="${IGNITE_HOME}" \
                -DIGNITE_PROG_NAME="$0" ${JVM_XOPTS} -cp "${CP}" ${MAIN_CLASS} > nohup.out 2>&1 &
            ;;
        esac
    else
        case $osname in
            ...
            *)
                nohup "$JAVA" ${JVM_OPTS} ${QUIET} "${RESTART_SUCCESS_OPT}" ${JMX_MON} \
                  -DIGNITE_HOME="${IGNITE_HOME}" \
                 -DIGNITE_PROG_NAME="$0" ${JVM_XOPTS} -cp "${CP}" ${MAIN_CLASS} "${CONFIG}" > nohup.out 2>&1 &
            ;;
        esac
    fi
